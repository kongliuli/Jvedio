...
| **扫描完成后立即加载新影片** | 扫描成功后自动刷新主界面列表 |
| **导入新影片后自动刮削元数据** | 新入库且有 VID、缺 metadata 的影片自动入队刮削（需开启「同步信息」） |
| **并行 Discover（大目录）** | 多线程目录枚举，适合文件很多的路径 |
| **目录指纹增量缓存** | 目录未变更时复用上次的文件列表，加快增量扫描 |
| **Discover 线程数** | 并行 Discover 的工作线程数；0 表示自动（约 CPU×2） |
| **Discover 预检** | 设置页按钮：仅枚举文件并显示数量/耗时/是否命中缓存，不入库 |
| **Discover 扫描（不入库）** | 设置页按钮：Discover 模式完整跑一遍，不写入数据库 |

**ScanMode 说明（开发者/API）**

| 模式 | 入口 | 行为 |
|------|------|------|
| `FullImport` | 常规扫描 | Discover + 入库 Pipeline |
| `Discover` | 设置页 Discover 按钮 | 仅枚举/统计，不入库 |
| `RefreshMetadata` | 列表右键「刷新元数据」 | 仅 `MetadataEngine` 刮削，**不**创建扫描任务；API：`ScanEngine.RefreshMetadata` |

列表右键 **刷新元数据** 只更新已有条目的 metadata（NFO → 爬虫），不会扫描磁盘或导入新文件。

扫描完成后，完成弹窗与任务列表会显示 Discover 摘要（文件数、耗时、是否来自缓存）。
