...
namespace Jvedio.Mapper
{
    public class MetaDataMapper : BaseMapper<MetaData>
    {
        ...
        {
            return deleteDataByIds(idList, LibraryContext.Current.DataType);
        }

        public int deleteDataByIds(List<string> idList, DataType dataType)
        {
            if (idList == null || idList.Count == 0)
                return 0;
            int c1 = MapperManager.metaDataMapper.DeleteByIds(idList);
            int c2 = MetaDataDeleteStrategy.DeleteTypeSpecificRows(idList, dataType, c1);
