...
namespace Jvedio.Core.FFmpeg
{
    public class ScreenShotTask : AbstractTask
    {
        ...
        [Obsolete("Subscribe to LibraryEventBus.ScreenShotError")]
        public static Action<string> onScreenShotError;

        [Obsolete("Subscribe to LibraryEventBus.ScreenShotCompleted")]
        ...
        public static void ScreenShotVideo(Video video, bool gif = false)
        {
            ...
            screenShotTask.onError += (e, v) => NotifyScreenShotError((v as MessageCallBackEventArgs).Message);
            screenShotTask.onCompleted += (e, v) => NotifyScreenShotCompleted((e as ScreenShotTask).Success, dataID);
            AddToScreenShot(screenShotTask);
        }

        private static void NotifyScreenShotCompleted(bool success, long dataId)
        {
            LibraryEventBus.RaiseScreenShotCompleted(success, dataId);
            onScreenShotCompleted?.Invoke(success, dataId);
        }

        private static void NotifyScreenShotError(string message)
        {
            LibraryEventBus.RaiseScreenShotError(message);
            onScreenShotError?.Invoke(message);
        }
        ...
        public static void AddToScreenShot(ScreenShotTask task)
        {
            ...
            App.TaskHub.AddTask(task, TaskKind.Generate);
        }
