    ...
    public class CrawlerManager
    {
        ...
        public static void Init(bool moveFile)
        {
            ...
            foreach (string crawler_dir in list) {
                ...
                if (!CrawlerPluginSecurity.VerifyDll(dllPath)) {
                    Logger.Warn($"crawler dll rejected by security policy: {dllPath}");
                    continue;
                }
        ...
        private static PluginMetaData GetPluginData(string dllPath)
        {
            ...
            if (classType == null || !CrawlerPluginSecurity.IsAllowedPublicType(classType)) {
                Logger.Warn($"crawler public type not allowed: {dllPath}");
                return null;
            }
