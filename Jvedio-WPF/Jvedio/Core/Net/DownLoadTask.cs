...
namespace Jvedio.Core.Net
{
    ...
    public class DownLoadTask : AbstractTask
    {
        ...
        #region "事件"
        [Obsolete("Subscribe to LibraryEventBus.DownloadCompleted")]
        public static Action<DownLoadTask> onDownloadSuccess;
        [Obsolete("Subscribe to LibraryEventBus.DownloadPreview")]
        ...
        /// <summary>仅刮削元数据，跳过海报/缩略图/演员/预览图下载。</summary>
        ...
        {
            // downLoader 保留给图片下载步骤；元数据统一走 MetadataEngine
            ...
            if (OverrideInfo || video.ToDownload()) {
                ...
                try {
                    dict = await MetadataEngine.RefreshAsync(
                        video, Token, Logger, header, (h) => { headerCallBack?.Invoke(h); });
                ...
                if (imageUrls != null && imageUrls.Count > 0) {
                    ...
                    for (int i = 0; i < imageCount; i++) {
                        ...
                        if (!File.Exists(saveFileName)) {
                            ...
                            if (fileByte != null && fileByte.Length > 0) {
                                FileHelper.ByteArrayToFile(fileByte, saveFileName);
                                NotifyDownloadPreview(DataID, saveFileName, fileByte);
        ...
        public override void DoWork()
        {
            Task.Run(async () => {
                ...
                await FinishWorkAsync("7. 同步所有内容完成");
            });
        }

        private async Task FinishWorkAsync(string statusText)
        {
        ...
        {
            if (MetadataScrapeService.Enqueue(video) == 0)
        ...
        {
            LibraryEventBus.RaiseDownloadCompleted(this);
            onDownloadSuccess?.Invoke(this);
        }

        private static void NotifyDownloadPreview(long dataId, string path, byte[] fileByte)
        {
            LibraryEventBus.RaiseDownloadPreview(dataId, path, fileByte);
            onDownloadPreview?.Invoke(dataId, path, fileByte);
        }
    }
}
