...
namespace Jvedio.Core.Tasks
{
    public class ScreenShotManager : BaseManager
    {
        ...
        private static TaskConfig BuildDefaultConfig()
        {
            return new TaskConfig() {
                TaskDelay = TASK_DELAY,
                TaskCount = (uint)TaskParallelismConfig.ResolveGenerateCount(),
        ...
        /// <summary>默认的任务配置</summary>
        private static TaskConfig DEFAULT_CONFIG { get; set; } = BuildDefaultConfig();
        ...
        {
            DEFAULT_CONFIG.TaskCount = (uint)TaskParallelismConfig.ResolveGenerateCount();
