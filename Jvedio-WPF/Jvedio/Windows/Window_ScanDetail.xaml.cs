...
namespace Jvedio
{
    ...
    public partial class Window_ScanDetail : BaseWindow
    {
        ...
        private DataType _libraryDataType;
        ...
        public Window_ScanDetail(ScanResult scanResult, DataType dataType) : this()
        {
            ScanResult = scanResult;
            _libraryDataType = dataType;
            Title = $"{LangManager.GetValueByKey("ImportDetail")} [{DataTypeDisplay.GetLabel(dataType)}]";
        }

        public Window_ScanDetail(ScanResult scanResult) : this(scanResult, LibraryContext.Current.DataType)
        {
        }
        ...
        private void BaseWindow_ContentRendered(object sender, EventArgs e)
        {
            ...
            if (discoverInfo != null) {
                string discoverText = ScanResult.FormatDiscoverSummary();
                discoverInfo.Text = discoverText ?? "-";
            }

            if (failNfoBorder != null)
                failNfoBorder.Visibility = _libraryDataType == DataType.Video ? Visibility.Visible : Visibility.Collapsed;
        }
