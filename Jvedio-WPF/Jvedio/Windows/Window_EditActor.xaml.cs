...
namespace Jvedio
{
    ...
    {
        #region "事件"
        [Obsolete("Subscribe to LibraryEventBus.ActorInfoChanged")]
        ...
        private void SaveActor(object sender, RoutedEventArgs e)
        {
            ...
            if (ActorID > 0) {
                ...
                if (update > 0) {
                    ...
                    NotifyActorInfoChanged(ActorID);
                }
            } else {
                // 新增
                // 检查是否存在
        ...
        private void ActorImage_Drop(object sender, DragEventArgs e)
        {
            ...
            if (dragdropFiles != null && dragdropFiles.Length > 0) {
                System.Collections.Generic.List<string> list = dragdropFiles.Where(arg => ScanExtensions.PICTURE_EXTENSIONS_LIST.Contains(System.IO.Path.GetExtension(arg).ToLower())).ToList();
                if (list.Count > 0) {
        ...
        private void SetCurrentImage(string targetFileName)
        {
            ...
            NotifyActorInfoChanged(CurrentActorInfo.ActorID);
        }
        ...
        private static void NotifyActorInfoChanged(long actorId)
        {
            LibraryEventBus.RaiseActorInfoChanged(actorId);
            onActorInfoChanged?.Invoke(actorId);
        }
    }
}