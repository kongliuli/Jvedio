...
namespace Jvedio
{
    public partial class WindowStartUp : BaseWindow, IBaseWindow
    {
        ...
        private ScanJobBase startupScanJob { get; set; }
        ...
        {
            LibraryContext.Apply(StartupLibraryMapping.ResolveDataType(ConfigManager.StartUp.SideIdx, ConfigManager.StartUp.LibraryDataType));
        ...
        private void NewDatabase(object sender, RoutedEventArgs e)
        {
            ...
            appDatabase.DataType = StartupLibraryMapping.ResolveDataType(vieModel.CurrentSideIdx, (int)appDatabase.DataType);
        ...
        private void ChangeDataType(object sender, RoutedEventArgs e)
        {
            ...
            LibraryContext.Apply(StartupLibraryMapping.ResolveDataType(idx, ConfigManager.StartUp.LibraryDataType));
            ConfigManager.StartUp.SideIdx = idx;
            ConfigManager.StartUp.LibraryDataType = StartupLibraryMapping.ToStorageValue(LibraryContext.Current.DataType);
            vieModel.ReadFromDataBase();
        }
            ...
            if (database == null) {
                ...
                if (main == null &&
                    ...
                    if (!string.IsNullOrEmpty(database.ScanPath)) {
                        ...
                        try {
                            IEnumerable<string> fileExt = LibraryContext.Current.DataType == DataType.Video
                                ? ScanExtensions.VIDEO_EXTENSIONS_LIST
                                : null;
                            DiscoveryMode discoveryMode = ConfigManager.ScanConfig.EnableDirIndexCache
                                ? DiscoveryMode.Incremental
                                : DiscoveryMode.Full;
                            startupScanJob = ScanFactory.ProduceScanner(LibraryContext.Current.DataType, toScan, null, fileExt, discoveryMode);
                            startupScanJob.onScanning += (s, ev) => {
                                Dispatcher.Invoke(() => {
                                    vieModel.LoadingText = (ev as MessageCallBackEventArgs).Message;
                                });
                            };
                            App.TaskHub.AddTask(startupScanJob, TaskKind.Scan);
                        } catch (Exception ex) {
                            Logger.Error(ex);
